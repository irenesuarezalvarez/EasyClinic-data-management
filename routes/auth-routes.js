...
router.post('/signup', (req, res, next) => {
    ...
    if (!username || !email || !role || !color || !password ) {
      return res.status(401).json({ errorMessage: 'All fields are mandatory. Please provide your username, role, email and password.' });
    }
   
    const regex = /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/;
    if (!regex.test(password)) {
      return res.status(401).json({ errorMessage: 'Password needs to have at least 6 chars and must contain at least one number, one lowercase and one uppercase letter.' });
    } 
...
router.post('/login', (req, res, next) => {
  ...
  if (email === '' || password === '') {
      return res.status(401).json({errorMessage: 'Please enter both, email and password to login.'});
  }
  ...
  Professional.findOne({ email })
    .then(professional => {
      ...
      if (!professional) {
        return res.status(401).json({ errorMessage: 'Email is not registered. Try with other email.' });
